...
int DS28E16_cmd_readStatus(
	unsigned char *
		data); // 读入芯片的页面保护属性、一个MAN_ID字节和一个芯片版本字节，合计6个字节
int DS28E16_cmd_readMemory(
	int pg,
	unsigned char *
		data); // pg=0,1,2页面，一次读入32个字节，前16个字节为有效数据，后续16个字节固定为0x00或0xFF
int DS28E16_cmd_writeMemory(
	int pg, unsigned char *data); // pg=0,1,2页面，一次必须完整写入16个字节
int DS28E16_cmd_decrementCounter(void); // 执行此函数，完成17位计数器减1操作
int DS28E16_cmd_setPageProtection(int pg,
				  unsigned char prot); // 设置芯片的页面属性
int DS28E16_cmd_device_disable(int op,
			       unsigned char *password); // 芯片自毁，不能恢复
int DS28E16_cmd_computeReadPageAuthentication(
	int anon, int pg, unsigned char *challenge,
	unsigned char *
		hmac); // 器件基于会话密码、随机数、页面数据和ROMID，返回器件的算法结果
int DS28E16_cmd_computeS_Secret(
	int anon, int bdconst, int pg,
	unsigned char
		*partial); // 基于主机发送的密钥种子，器件在内部计算出会话密码
